    ...
    class Physics:        #物理系统
        ...
        class RigidBody:  #刚体
            def __init__(self,core,rigid_body_config=None):
                self.core = core
                self.config = core.config
                self.core.Event.on("apply_force_to_rigidbody",self._on_apply_force)
                self.core.Event.on("change_rigidbody_property",self._on_change_property)
                self.core.Event.on("collision_rigidbody",self._on_collision)
                #物理基础属性
                self.mass = 1.0                                       #质量
                self.volume = 0.1                                     #体积
            ...
            def _on_change_property(self,data):        #改变属性事件接受方法
                ...
                if hasattr(self, prop_name):
                    ...
                    #更新密度
                    if prop_name in ["mass","volume"]:
                        self.density = self.mass / self.volume
            def _update(self,delta_time):           #每帧更新，由物理系统调用
                #应用空气阻力
                if self.velocity.length() > 0:
                    drag = -self.velocity.normalize() * self.air_resistance
